...
export class NgxUiLoaderService {
   ...
   */
  getLoader(loaderId?: string): Loader | null {
  ...
  private clearTimers(task: Task): void {
    clearTimeout(task.delayTimer as any);
    clearTimeout(task.maxTimer as any);
    clearTimeout(task.minTimer as any);
  }
  ...
  private readyToStart(
    ...
    this.createLoaderData(loaderId, undefined ?? false, false);
